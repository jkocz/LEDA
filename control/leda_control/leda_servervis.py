...
def onMessage(ledavis, message, clientsocket, address):
	...
	if 'open' in args:
		...
		if visibilities is None:
			...
			"""
			amp_xx    = 10*np.log10(np.mean(np.abs(matrix_xx), axis=0))
			amp_yy    = 10*np.log10(np.mean(np.abs(matrix_yy), axis=0))
			phase_xx  = np.mean(np.angle(matrix_xx), axis=0)
			phase_yy  = np.mean(np.angle(matrix_yy), axis=0)
