...
class XGPUReader(object):
	...
	def __init__(self, headerdict):
		...
		if self.orderstr.find('TRIANGULAR') == 0:
			...
			self.rawshape = self.rawshape_triangular
			...
			self.rawshape = self.rawshape_real_imag_triangular
			...
			self.rawshape = self.rawshape_reg_tile_triangular
	...
	def rawshape_triangular(self):
		return (self.nbin,self.nchan,self.nbaseline,self.npol,self.npol)
	def rawshape_real_imag_triangular(self):
		# Note: rawdata is ordered (ndim, nchan, nbaseline, npol, npol)
		return (2,self.nbin,self.nchan,self.nbaseline,self.npol,self.npol)
	def rawshape_reg_tile_triangular(self):
		nbaseline = self.reg_tile_triangular_nbaseline()
		return (2,self.nbin,self.nchan,nbaseline,self.npol,self.npol)
	# TODO: Should be made into properties?
	def rawsize(self):
		"""Returns (expected) size in bytes of raw input data
		"""
		rawshape = self.rawshape()
		itemsize = np.dtype(self.dtype).itemsize
		return reduce(lambda a,b:a*b, rawshape, 1) * itemsize
