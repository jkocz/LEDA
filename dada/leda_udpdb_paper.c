...
int leda_udpNdb_close_hdu (udpNdb_t * ctx, uint64_t bytes_written)
{
  ...
  if ((ctx->verbose > 1) || (ctx->verbose && stop_pending))
    multilog (ctx->log, LOG_INFO, "leda_udpNdb_close_hdu(%"PRIu64")\n", bytes_written);
  ...
  if (ctx->block_open)
  {
    if ((ctx->verbose && stop_pending) || (ctx->verbose > 1))
      multilog (ctx->log, LOG_INFO, "close_hdu: leda_udpNdb_close_buffer(%"PRIu64", 1)\n", bytes_written);
    if (leda_udpNdb_close_buffer (ctx, bytes_written, 1) < 0)
    {
      multilog (ctx->log, LOG_INFO, "close_hdu: leda_udpNdb_close_buffer failed\n");
      return -1;
    }
  }
 ...
 */
int leda_udpNdb_open_buffer (udpNdb_t * ctx)
{
  ...
  ctx->block = ipcio_open_block_write (ctx->hdu[ctx->ihdu]->data_block, &block_id);
  if (!ctx->block)
  { 
 ...
 */
int leda_udpNdb_close_buffer (udpNdb_t * ctx, uint64_t bytes_written, unsigned eod)
{
 ...
 */
int leda_udpNdb_new_buffer (udpNdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "leda_udpNdb_new_buffer()\n");

  if (leda_udpNdb_close_buffer (ctx, ctx->hdu_bufsz, 0) < 0)
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: leda_udpNdb_close_buffer failed\n");
    return -1;
  }

  if (leda_udpNdb_open_buffer (ctx) < 0) 
  {
 ...
 */
int lead_udpNdb_open_hdu (udpNdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "leda_udpNdb_open_hdu()\n");
  ...
  if (leda_udpNdb_open_buffer (ctx) < 0)
  {
    multilog (ctx->log, LOG_ERR, "open_hdu: leda_udpNdb_open_buffer failed\n");
    return -1;
  }
 ...
 */
int leda_udpNdb_new_hdu (udpNdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog(ctx->log, LOG_INFO, "leda_udpNdb_new_hdu()\n");

  if (leda_udpNdb_close_hdu (ctx, ctx->hdu_bufsz) < 0)
  {
    multilog(ctx->log, LOG_ERR, "new_hdu: leda_udpNdb_close_hdu failed\n");
    return -1;
  }
  ...
  if (leda_udpNdb_open_hdu (ctx) < 0)
  {
    multilog(ctx->log, LOG_ERR, "new_hdu: leda_udpNdb_open_hdu failed\n");
    return -1;
  }
