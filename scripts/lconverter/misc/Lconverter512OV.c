...
int main(int argc, char** argv) {
  ...
  unsigned long filenameOffset = 1151881216;
  ...
  for (outerL=0;outerL<sliceToRead/slicePerFile;outerL++)
  {
        ...
        for (innerL=0;innerL<slicePerFile;innerL++)
	{
		...
		// write to output
		strideLength= 131328 + 130816; // (ninput*ninput+1)/2 + (ninput*ninput-1)/2
		int bytes_printed = 0;
                acount = 0;
                aflip = 0;
                ccount = 2;
			...
			{	
				...
				fwrite(&full_matrix_h[acount+strideLength*freqCh].real,1,4,outputFileA);
                        ...
                        else
                        {
                                //acount +=65;
                                //acount +=129;
	 			acount +=1025; //(ninput *2) + 1
                                aflip = 0;
                        }
		}
		//fprintf (stdout, "AC writen, bytes printed: %d\n",bytes_printed);
		for (int ccIndex=0;ccIndex<512;ccIndex++)
		{
                        for (int xcount=ccIndex;xcount<511;xcount++)
                        {
